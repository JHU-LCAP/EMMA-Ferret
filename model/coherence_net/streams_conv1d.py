...
class Stream2(torch.nn.Module):
    def __init__(
        ...
        self.attention = Gate_Attention(input_channels = num_sources,
                                        ...
                                        time_frames = time_frames)
        self.conv_final = torch.nn.Sequential(
            torch.nn.Conv1d(embedding_dim + dcs_num_layers*dcs_output_dim, 1, kernel_size = 1, padding = 'same'), 
            torch.nn.ReLU(),
        )
    ...
    def forward(self, xs: torch.Tensor, x_indicator: torch.Tensor):
        ...
        upsample_xs = self.upsample(dcs_xs, output_size = conv1_xs.shape) ## [B, embedding_dim, Freq_contetn, Time_frames]
        ...
        embedding_fin = embedding.view(B, 512, -1)
        print(embedding_fin.shape, "emb fin")
        final_predictions = self.conv_final(embedding_fin)
        final_predictions = final_predictions.view(B, 1, 128, -1)
        print(final_predictions.shape, "final preds") 
